---
title: "Parallel Random Forest in R"
...
This tutorial will give an example of how to parallelize the code for running a Random Forrest Regression using the "parallel" and "randomForest" packages in R. A comparison between the parallel code and the regular code will be made using the "microbenchmark" package. My purpose in this exercise was not any sort of predictive accuracy of the model but to demonstrate this particular use case for the "Parallel" package.   Other tutorials about how and why to use code parallelization for different tasks in R can be found at: https://52.61.179.13/post/329/   and   https://52.61.179.13/post/356/ 
...
### Parallel Random Forest
 ...
 The Random Forest algorithm is a good candidate for parallel computing as the re-sampling has no requirement to be done sequentially. If your computer has multiple cores the re-sampling work can been split equally between them. The Random Forest above was fit with 9000 trees sequentially. Using the parallel package this this can be split up equally between the available cores. 
...
The results from the two models are roughly the same. However, you will notice that the parallel fit version of the model has about a 50% faster run-time than the sequentially fit model. 
